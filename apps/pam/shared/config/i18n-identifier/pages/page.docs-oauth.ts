 ...
 * @localZh 必填：response_type=code、client_id、redirect_uri（须在应用白名单内）。可选：scope、state、ui_locales（或 locale，用于授权页语言，如 zh/en）；公共客户端必填 code_challenge 与 code_challenge_method=S256。
 * @localEn Required: response_type=code, client_id, redirect_uri (must match a registered URI). Optional: scope, state, ui_locales (or locale, for consent UI language, e.g. zh/en); public clients require code_challenge and code_challenge_method=S256.
 */
