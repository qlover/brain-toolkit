-- Extend pam_search_projects with sort-by (created_at | updated_at).
-- Safe to re-run.

CREATE INDEX IF NOT EXISTS idx_pam_projects_list_active_updated
  ON pam_projects (is_public DESC, updated_at DESC, id DESC)
  WHERE is_deleted = 0;
