...
export function AdminMemoryKvPanel({ tt }: { tt: AdminMemoryKvI18nInterface }) {
  ...
  const [list, listStore] =
    usePendingAsyncStore<AsyncState<PamMemoryKvListResult>>();
  const [purge, purgeStore] =
    useAsyncStore<AsyncState<PamMemoryKvPurgeResult>>();
  ...
  const columns: TableColumn<PamMemoryKvAdminEntry>[] = [
    {
      title: tt.colKey,
      dataIndex: 'key',
      key: 'key',
  ...
  if (authLoading) {
    return <AdminPanelLoading testId="AdminMemoryKvLoading" />;
  }
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="AdminMemoryKvPanel">
      ...
      {loading && entries.length === 0 ? (
        <AdminPanelLoading testId="AdminMemoryKvLoading" />
      ) : (
        <Table
          rowKey="key"
