...
function toDraft(item: PamAdminLocaleItem): DraftLocale {
  return {
    id: item.id,
    value: item.value,
...
export function AdminLocalesPanel({ tt }: { tt: AdminLocalesI18nInterface }) {
  ...
  const columns: TableColumn<PamAdminLocaleItem>[] = useMemo(
    () => [
      {
        title: tt.colValue,
        dataIndex: 'value',
  ...
  if (authLoading) {
    return <AdminPanelLoading testId="AdminLocalesAuthLoading" />;
  }
  ...
  return (
    <div data-testid="AdminLocalesPanel" className="flex flex-col gap-4">
      ...
      {loading && rows.length === 0 ? (
        <AdminPanelLoading testId="AdminLocalesLoading" />
      ) : (
        <Table
          rowKey="id"
          loading={loading}
