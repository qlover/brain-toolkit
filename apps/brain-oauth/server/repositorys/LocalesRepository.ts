...
/**
 * `useApiLocales` is `false` in `@config/common`, so this repository is not
 * exercised at runtime; methods are stubbed to match the kit's `SupabaseRepo`
 * shape until DB-backed locale editing is needed.
 */
@injectable()
