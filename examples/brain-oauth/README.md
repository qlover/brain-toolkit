...
**TL;DR**：`npm install` → 将 `.env.template` 复制为 `.env` 并按注释填写（OAuth 相关变量见下文）→ 在 Supabase 执行 `makes/sql/` 脚本 → `npm run dev`（默认端口 **3112**，`APP_ENV=localhost`）→ 生产：`npm run build` 后 `npm start`（默认端口 **3111**）。

**文档**：站内 OAuth 集成说明见 [`/[locale]/docs/oauth`](./src/app/[locale]/docs/oauth/page.tsx)（开发环境如 `http://localhost:3112/zh/docs/oauth`）；国际化约定见 [docs/i18n.md](./docs/i18n.md)。
...
1. 打开 `http://localhost:3112`（或带 locale 的首页，如 `/zh`）。
...
1. 部署本示例并配置 `SITE_URL`（如 `http://localhost:3112`）。
...
```env
OAUTH_SITE_URL=http://localhost:3112
...
1. `npm run dev` 启动 OAuth 站点（3112）
...
3. **终端用户登录（本站点）**  
   ...
   ```bash
   curl -X POST http://localhost:3112/api/oauth/verify \
...
6. **Userinfo**  
   ...
   ```bash
   curl http://localhost:3112/userinfo \
...
**总结**：**`shared/oauth-wrapper`** 实现可复用的 OAuth 2.0 授权服务器逻辑；本示例通过 **Provider + Adapter + Repository + Session** 接入 Brain User API，并暴露标准端点。开发时用 **`npm run dev`（3112）** 即可同时调试页面、Playground 与 `/oauth/token`、`/userinfo`。
