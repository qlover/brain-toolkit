...
**TL;DR**: `npm install` → copy `.env.template` to `.env` (OAuth variables below) → run `makes/sql/` on Supabase → `npm run dev` (port **3112**, `APP_ENV=localhost`) → production: `npm run build` then `npm start` (port **3111**).

**Docs**: In-app OAuth guide at `/[locale]/docs/oauth` (e.g. `http://localhost:3112/en/docs/oauth`); i18n conventions in [docs/i18n.en.md](./docs/i18n.en.md).
...
1. Set `SITE_URL` (e.g. `http://localhost:3112`).
...
5. Backend exchanges the code:
   ...
   ```bash
   curl -X POST http://localhost:3112/oauth/token \
...
6. Call userinfo:
   ...
   ```bash
   curl http://localhost:3112/userinfo -H "Authorization: Bearer ACCESS_TOKEN"
   ```
...
Pages and Gateways call `/api/*`; third parties call `/oauth/token` and `/userinfo` on the same origin. `npm run dev` serves everything on port **3112**.
...
**Summary:** **`shared/oauth-wrapper`** is the reusable OAuth server core; this example wires **Brain User** via `BrainUserAdapter` and Supabase. Use **`npm run dev`** on port **3112** to exercise the full flow including Playground and machine endpoints.
