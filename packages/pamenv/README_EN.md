...
```bash
pamenv config set locale zh   # locks locale; pulls `api` error messages
pamenv locales pull           # refresh API error catalog only
```

- **CLI UX** (`pamenv_cli:*`): generated by ts2locales into `dist/locales`, loaded at runtime; use identifier constants (e.g. `PAMENV_CLI_LOGIN_WAITING`).
- **API errors** (`api:*`): pulled from PAM `/api/locales/json?namespaces=api` into `config.localeMessages`.
- Browser device login sends the page locale; if not locked, CLI adopts it.

After editing identifier JSDoc, run `pnpm --filter pamenv-cli build` (or `pnpm gen:locales`).
