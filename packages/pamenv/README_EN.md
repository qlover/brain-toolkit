...
Local files default to `.env.<environment>` (e.g. `-e local` → `.env.local`). Use `--file .env` to override (relative to `-o`/cwd or absolute). Without `-e`, the first environment in the list is used (`remove` always requires `-e`).

### Config and locales

```bash
pamenv config set locale zh   # locks locale
pamenv locales pull           # refresh PAM api:* error catalog (memory only)
```

- **CLI UX** (`pamenv_cli:*`): generated by ts2locales into `dist/locales`, loaded locally at runtime; use identifier constants (e.g. `PAMENV_CLI_LOGIN_WAITING`). After editing identifier JSDoc, run `pnpm --filter pamenv-cli build` (or `pnpm gen:locales`).
- **API errors** (`api:*`): fetched from PAM `/api/locales/json?namespaces=api` into memory at command start. Not written to `config.json`.
- Browser device login sends the page locale; if not locked, CLI adopts it and refetches `api` copy.
